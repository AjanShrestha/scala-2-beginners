...
object HOFsCurries extends App {
  ...
  private def toCurry(f: (Int, Int) => Int): Int => Int => Int =
    x => y => f(x, y)

  private def fromCurry(f: (Int => Int => Int)): (Int, Int) => Int =
    (x, y) => f(x)(y)

  // FunctionX
  private def compose[A, B, T](f: A => B, g: T => A): T => B =
    x => f(g(x))

  private def andThen[A, B, C](f: A => B, g: B => C): A => C =
    x => g(f(x))

  private def superAdder2: (Int => Int => Int) = toCurry(_ + _)

  private def add4: Int => Int = superAdder2(4)
  ...
  private val add2 = (x: Int) => x + 2
  private val times3 = (x: Int) => x * 3

  private val composed = compose(add2, times3)
  private val ordered = andThen(add2, times3)
